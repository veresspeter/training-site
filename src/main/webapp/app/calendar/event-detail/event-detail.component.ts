...
export class EventDetailComponent implements OnInit, OnDestroy {
  ...
  switchMuteButton(uid: string): void {
    ...
    if (muteButton?.classList.contains('muted')) {
      this.agora.client?.remoteUsers?.find(user => EventDetailComponent.getUserIdFromUID(user.uid) === uid)?.audioTrack?.play();
      muteButton.classList.add('btn-outline-primary');
      muteButton.classList.remove('muted');
      muteButton.classList.remove('btn-primary');
    } else {
      this.agora.client?.remoteUsers?.find(user => EventDetailComponent.getUserIdFromUID(user.uid) === uid)?.audioTrack?.stop();
